...
async function submitMenulis(letter) {
  ...
  const imgData = quizCanvas.toDataURL('image/png').split(',')[1];
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: 'text',  text:
`Anak PAUD sedang belajar menulis huruf "${letter}". Ini gambar hasil tulisannya.
Nilai apakah tulisan ini adalah huruf ${letter}.
Balas HANYA JSON: {"correct":true,"similarity":75,"message":"umpan balik positif 1 kalimat bahasa Indonesia untuk anak"}
Jika kanvas kosong: correct=false, similarity=0.` }
          ]
        }]
      })
    });
    const data = await resp.json();
    const res  = JSON.parse((data.content[0]?.text || '{"correct":false,"similarity":0,"message":"Coba lagi ya!"}').replace(/```json|```/g,'').trim());
    fbText.textContent = res.message;
    if (res.correct || res.similarity >= 55) {
      quizScore++; quizStreak++;
      if (quizStreak > bestStreak) bestStreak = quizStreak;
